...
mod tests {
    ...
    /// Tests that the `From<TryFromIntError>` implementation correctly creates
    /// `RecvError::MessageTooLarge`. This error occurs when `recv_handshake`
    /// receives a length header that exceeds `usize::MAX` (relevant on 32-bit
    /// platforms where `u64` can exceed `usize`).
    #[test]
    fn recv_error_message_too_large_from_try_from_int_error() {
        use std::num::TryFromIntError;

        // Create a TryFromIntError by attempting an impossible conversion
        // (i8::MAX can't fit in u8 when cast from a negative value scenario,
        // but we use a simpler approach: -1i8 to u8)
        let err: Result<u8, TryFromIntError> = (-1i8).try_into();
        let try_err = err.unwrap_err();

        // Verify the conversion to RecvError::MessageTooLarge works
        let recv_err: RecvError = try_err.into();
        assert!(
            matches!(recv_err, RecvError::MessageTooLarge(_)),
    ...
    fn error_display_messages_are_descriptive() {
        ...
        let result: Result<u8, TryFromIntError> = (-1i8).try_into();
        let recv_err: RecvError = result.unwrap_err().into();
